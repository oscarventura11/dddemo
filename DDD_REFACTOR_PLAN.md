# Refactor Plan: From 'Bad Example' to DDD & Hexagonal Architecture
...
4.  **Testing Difficulties:**
    *   Since the logic is inside a React/Preact hook, it's difficult to test the business rules in isolation without mocking the entire rendering cycle.
...
| :--- | :--- | :--- |
| **Domain** | Pure business logic, Entities (Composite Pattern), and Abstract Classes. | `Category` Entity, `CategoryRepository` (Port) |
| **Application** | State management and CQRS (Read/Write). | `CategoryReadService`, `CategoryWriteService`, `CategoryState` |
| **Infrastructure**| External implementations (API, JSON, Storage). | `CategoryJsonRepository` (Adapter), `CategoryMapper` |
...
| **Arch. Validation** | ts-arch | `ts-arch` |
...
### 1. Robust Domain Model (Composite Pattern)
Instead of a simple hook, we will have a `Category` Entity using the **Composite Pattern**.
*   **Business Rule:** "When a parent is selected, all children are selected."
*   **Implementation:** The `Branch` (parent) will propagate the selection to its children.

### 2. CQRS in Application Layer
We separate the application logic into three distinct services:
*   **ReadService:** Responsible for fetching data and populating the state.
*   **WriteService:** Responsible for executing commands (e.g., toggling a category, creating a new node).
*   **StateService:** The single source of truth for the feature's reactive state (Signals).

### 3. Dependency Injection with Inversify
We will use `@injectable()` and `@inject()` to decouple our code.
*   Services won't know *where* the data comes from (JSON file, API, or Mock). They only know the `CategoryRepository` abstract class.

### 4. Testability
*   **Domain Tests:** Test `Category` entity logic in pure TS without any UI.
*   **Application Tests:** Test Read/Write services using `ts-mockito` to mock the repository.
*   **Architecture Tests:** Use `ts-arch` to ensure that `Domain` never imports from `Infrastructure` or `Presentation`.
...
└── ddd/
    ...
    │   ├── entities/          # Category.ts (Composite)
    │   ├── value-objects/     # CategoryId.ts
    │   └── repositories/      # CategoryRepository.ts
    ├── application/           # CQRS & State
    │   ├── services/          # CategoryReadService.ts, CategoryWriteService.ts
    │   └── state/             # CategoryState.ts (Signals)
    ├── infrastructure/        # External Implementations
    │   ├── repositories/      # CategoryJsonRepository.ts
    │   ├── mappers/           # CategoryMapper.ts
    │   └── di/                # inversify.config.ts
    └── presentation/          # UI Components
        ├── containers/        # CategoryTreeContainer.tsx (Smart)
        └── components/        # CategoryComponent.tsx (Dumb)
```
